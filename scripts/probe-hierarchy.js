...
function load(name) {
    ...
    console.log(`  [load] ${name}: ${geo.features.length} features in ${Math.round(performance.now() - t0)}ms`);
    return geo;
}
...
function validateContainment(result, levels) {
    ...
    for (const a of result.assignments) {
        ...
        for (let i = 0; i < a._path.length; i++) {
            ...
            if (!block || !rectContainsCell(block, cell.r, cell.c)) { inside = false; break; }
        }
        if (!inside) { bad++; if (violations.length < 5) violations.push({ name: a.name, path: a._path, cell }); }
    }
...
async function runVillageLevel(features, cap, label) {
    ...
    const points = data.map(f => {
        ...
        return { name: f.properties.name, code: f.properties.code, provinsi_code: f.properties.provinsi_code, kab_kota_code: f.properties.kab_kota_code, kecamatan_code: f.properties.kecamatan_code, x, y };
    });
    ...
    console.log(`  grid ${result.meta.rows}x${result.meta.cols} (${result.meta.gridCells} cells), used ${result.meta.usedCells}`);
    console.log(`  solved in ${dt}ms  (${points.length} leaves)`);

    const v = validateContainment(result, ['provinsi_code', 'kab_kota_code', 'kecamatan_code']);
    console.log(`  containment: ${v.total - v.bad}/${v.total} inside ancestor blocks` + (v.bad ? `  BAD=${v.bad}` : '  ✓') + `  (subdivided leaf blocks: ${v.subdivided})`);
...
async function runKecamatanLevel() {
    ...
    const points = geo.features.map(f => {
        const [x, y] = centroid(f);
        return { name: f.properties.name, code: f.properties.code, provinsi_code: f.properties.provinsi_code, kab_kota_code: f.properties.kab_kota_code, x, y };
    });
    ...
    for (const [id, r] of blocks) {
        features.push({
            ...
            geometry: {
                type: 'Polygon',
                coordinates: [[
                    [r.c0, r.r0], [r.c1 + 1, r.r0], [r.c1 + 1, r.r1 + 1], [r.c0, r.r1 + 1], [r.c0, r.r0]
                ]]
            }
        });
    }
...
async function main() {
    ...
    const cap = process.argv[2] && !isNaN(Number(process.argv[2])) ? Number(process.argv[2]) : null;
    ...
    const vres = await runVillageLevel(villages.features, cap ?? 12000, `villages (capped ${cap ?? 12000})`);
