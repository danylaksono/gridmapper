...
async function main() {
  ...
  for (const level of LEVELS) {
    ...
    try {
      ...
      console.log(
        `${level}\t${data.length}\t${gridResult.meta.rows}x${gridResult.meta.cols}\t${dt}\trect\t${gridResult.meta.isFeasible ? "optimal" : "infeasible"}`,
      );
    } catch (e) {
      console.log(
        `${level}\t${data.length}\t-\tERR\t-\t${e.message.slice(0, 80)}`,
      );
    }
      ...
      try {
        const geoWithW = {
          ...
          features: features.map((f, i) => ({
            ...f,
            properties: { ...f.properties, __w: i + 1 },
          })),
        };
        ...
        const dorl = createDorlingCartogram(geoWithW, "__w", {
          ...
          iterations: 400,
          k: 12,
          refineOverlaps: false,
          seed: 1,
        });
        const dt = Math.round(performance.now() - t1);
        console.log(
          `  dorling\t${data.length}\t-\t${dt}\tcircle\tfeatures=${dorl.features.length}`,
        );
      } catch (e) {
        console.log(
          `  dorling\t${data.length}\t-\tERR\t-\t${e.message.slice(0, 80)}`,
        );
      }
    } else {
      console.log(
        `  dorling\t${data.length}\t-\tskip\t-\t(O(n^2) too slow above ~3k)`,
      );
    }
  }
}
