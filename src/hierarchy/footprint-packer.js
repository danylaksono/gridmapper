...
export async function packLeavesIntoBlock(children, block, options = {}) {
  ...
  if (children.length <= smallBlockThreshold) {
    ...
    if (!mip) {
      throw new Error(
        "packLeavesIntoBlock: options.mip is required for blocks above smallBlockThreshold",
      );
    }
    const res = await mapper.allocate(
...
function greedyPack(children, rows, cols, xAccessor, yAccessor) {
  ...
  const order = children
    .map((c, i) => ({ c, i }))
    .sort((a, b) => {
      const ay = yAccessor(a.c.item),
        ax = xAccessor(a.c.item);
      const by = yAccessor(b.c.item),
        bx = xAccessor(b.c.item);
      return ay - by || ax - bx;
    });
